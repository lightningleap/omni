  ...
  return (
    <aside className="w-64 border-r border-slate-200 bg-[#FAF9F6] flex flex-col h-screen fixed left-0 top-0 z-[60]">
      ...
      <div className="p-8 pb-6">
        <div className="mb-0">
          <h1 className="text-2xl font-serif italic font-bold tracking-tighter text-accent-700">Unrwly</h1>
          ...
          return (
            ...
            >
              {isActuallyActive && (
                <motion.div
                  layoutId="activePill"
                  className="absolute left-0 w-1 h-5 bg-accent-800 rounded-full"
                  transition={{ type: "spring", stiffness: 300, damping: 30 }}
                />
              )}
              <item.icon size={16} className={`${isActuallyActive ? 'text-accent-700' : 'text-slate-400 group-hover:text-slate-600'}`} />
      ...
      <div className="p-6 border-t border-slate-200">
        <div className="space-y-2">
          <Link
            href="/"
            className="flex items-center gap-2 px-4 py-2 text-[11px] font-bold text-slate-500 hover:text-accent-700 hover:bg-accent-50 rounded-lg transition-all"
          >
